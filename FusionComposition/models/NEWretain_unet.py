...
class WeightedConcat(nn.Module):
    def __init__(self, num_channels):
        ...
        # Initialize the weights for each channel of the concatenated tensor
        self.weights = nn.Parameter(
            torch.randn(2 * num_channels)
        )  # Random initialization

    def forward(self, tensor1, tensor2):
        # Concatenate the tensors along the channel dimension
        concatenated = torch.cat([tensor1, tensor2], dim=1)
        # Normalize the weights using softmax to ensure they sum to 1
        normalized_weights = F.softmax(self.weights, dim=0)
        # Apply the normalized weights to each channel
        weighted = concatenated * normalized_weights.view(
            1, -1, 1, 1
        )  # Reshape weights to match the dimensions of B, C, H, W
        return weighted
...
class RetFuseNet(nn.Module):
    ...
    def forward(self, data):
        ...
        feats = [dual_conv(x) for dual_conv in self.dual_convs2]
