...
# def resample_point_clouds(root_dir, out_dir, max_points, min_points, glob="*.las", clusters_list=None):
#     files = list(Path(root_dir).glob(glob))
    
    
#     if not os.path.exists(out_dir):
#         os.makedirs(out_dir)
#     for file in tqdm(files, desc="Resampling Point Clouds", leave=False, colour="red"):
#         coords = read_las(file)
#         filename = str(file).split("\\")[-1]
#         num_points = random.randint(min_points, max_points)
#         clusters = random.choice(clusters_list)
#         kmeans = KMeans(n_clusters=clusters, random_state=0).fit(coords)
#         coords = cluster_fps(coords, kmeans.labels_, num_points)
#         write_las(coords, os.path.join(out_dir, filename))
...
if __name__ == "__main__":
    dataset_path = r"D:\MurrayBrent\projects\paper3\data\raw\RMF_ITD\all"
    resample_point_clouds(
        root_dir=dataset_path,
        max_points_list=[1024],
        samp_meth="cluster_fps",
        clusters=8
    )
# if __name__ == "__main__":
#     dataset_path = r"D:\MurrayBrent\projects\paper3\data\raw\TR3D_species\val\datasets\cluster_fps\4096"
#     output_path = r"D:\MurrayBrent\projects\paper3\data\raw\TR3D_species\val\datasets\small_pointclouds"
#     resample_point_clouds(
#         root_dir=dataset_path,
#         out_dir=output_path,
#         max_points = 3000,
#         min_points = 300,
#         clusters_list = [5, 6, 7, 8, 9, 10]
#     )